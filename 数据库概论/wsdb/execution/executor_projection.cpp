...
void ProjectionExecutor::Init() {
	...
	record_ = nullptr;
	if(IsEnd()) return;
	auto childRecord = child_->GetRecord();
	if(childRecord) {
		record_ = std::make_unique<Record>(out_schema_.get(), *childRecord);
	}
}
...
auto ProjectionExecutor::IsEnd() const -> bool { return child_->IsEnd(); }
